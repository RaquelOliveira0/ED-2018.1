...
void furar(matchar &mat, Par inicio){
    vector<Par> pilha;
    ...
    while (pilha.size()!=0 ) {
        vector<Par>aux;
        Par top =pilha.back();
        for(auto vizinhos : getNeib(top))
       if(countOpen(mat,vizinhos)<=1 && mat.equals(vizinhos,PAREDE))
        aux.push_back(vizinhos);
      if(aux.size()!=0){
      int par=rand()% aux.size();
        mat.get(aux[par]) = ABERTO;
        pilha.push_back(aux[par]);
        mat_draw(mat);
        ed_show();
